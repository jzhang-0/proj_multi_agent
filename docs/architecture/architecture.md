...
- **CLI**:`amux workspace add|list|rm|current`;`amux` 按 cwd 自动选工作区,`--workspace <slug>` 显式指定;`amux msg` 从 cwd 定位工作区总线。控制台标题栏显示当前 slug 与项目根,`/workspace <名字>` 切换绑定(成员栏与时间线跟着换)。IM 网关按房间名(或 `workspace` 字段)把消息投进对应工作区总线,白名单可按 `[workspaces.<slug>]` 分开放。`rm` 只删 `~/.amux` 里的登记和状态目录,不碰用户项目文件。
