...
- [ ] **BUS-001** — `src/bus/` 模块化:消息 schema v1(`to/from/text/ts` 必备 + 可选 `id/kind/replyTo`)定义与校验,畸形消息进死信目录不炸投递循环;bus 根目录路径可注入(默认仓库根 `bus/`,测试用临时目录)。
  - 处理登记:claude,2026-08-16 06:05 +0800,`bus-001-claude`。
  - 前置:无。
