...
- [ ] **CON-016** — 直连交互菜单方向键:选中成员、输入框为空且没有补全候选时,`↑`/`↓` 分别作为 tmux `Up`/`Down` 透传,用于操作 Codex 等成员终端里的审批菜单；`Ctrl+↑`/`Ctrl+↓` 继续回看成员 tmux 历史,群聊发送历史、成员/命令补全和非空输入保持原语义。动作需写入审计与时间线,同步输入提示、帮助、README 和产品定义,并用安全夹具完成实际画面验证。
  - 前置:CON-014、CON-015。
  - 处理登记:Codex，2026-08-23，分支 `con-016-codex`。
