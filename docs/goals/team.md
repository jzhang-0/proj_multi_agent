...
- [ ] **TEAM-008** — Leader 风险分级验收：运行时 Leader 提示必须把验收深度与任务风险、改动范围、可逆性和不确定性匹配。简单、局部、可逆任务采用最小充分验证，证据已覆盖直接改动就及时结项，不得惯性扩成全量测试、重复调查或多人独立评审；只有出现跨模块/公共契约、安全与数据风险、不可逆操作、失败信号或证据矛盾等具体理由时才升级验证，并说明升级原因。Leader 仍承担最终验收责任，不能把“轻量验证”解释为复述成员结论或不检查产物；向 human 的汇报深度同样与风险匹配。补运行时提示契约测试并同步权威产品/架构说明。
  - 前置：TEAM-002、TEAM-006。
  - 处理登记：Codex，2026-08-30 13:53 CST，分支 `team-008-codex`。
