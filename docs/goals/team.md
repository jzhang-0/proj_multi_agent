...
- [ ] **TEAM-005** — Claude 成员颜色保真：默认 `fable-core` 的 Fable/Sonnet/Opus 显式以空 `NO_COLOR` 覆盖 amux 启动环境中可能继承的 `NO_COLOR=1`，使 Claude 向 tmux 网格输出 ANSI 样式，再由现有 `capture-pane -e → Rich Text` 链路原样显示；Luna/Sol 及用户自定义非 Claude 成员不强制改变。补默认档案与激活投影测试、文档、真实 Claude pane 的有色捕获证据，并更新当前 `proj_fppt` 工作区成员配置；不擅自重启正在运行的 Claude 会话。
  - 前置：TEAM-004、TMX-004、CON-006。
  - 处理登记：Codex，2026-08-23，`team-005-codex`。
  - 进行中：已确认 amux 颜色捕获链正常，当前 Claude 原始捕获为 0 个 ANSI，临时 pane 在 `NO_COLOR` 为空时为 110 个，正在持久化团队启动环境。
