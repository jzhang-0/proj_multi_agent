...
- [ ] **WEB-007** — tmux 镜像、回滚与受限直连：实现「成员终端镜像」「成员终端回滚」「终端窗口适配」「成员画面点击直连」「直连文本输入」「直连编辑/方向键」。完整帧 WebSocket，≤10 Hz 合并/背压，ANSI 经随包终端组件安全渲染(禁 `innerHTML`/CDN)，canonical size 由租约持有者决定；浏览器行号须经服务端最新帧再次确认 composer；输入经 WEB-002 租约与 `tmuxctl` 串行。覆盖 TUI/Web 同时观看、抢占、回滚态拒绝输入、断线释放测试。
  - 处理登记:Sonnet，2026-08-30，分支 `web-007-sonnet`(T-015)，暂不合 main(等 WEB-005)。**后端半边已完成**:`src/web/terminal.py` 实现 `MirrorGroup`/`MirrorHub`(按 `(member, history_offset)` 单生产者+多消费者扇出，实时 10Hz/回滚 2Hz，`maxsize=1` 背压，静止 5s 发 idle 帧)、租约 acquire/连接驱动心跳/断线立即释放、resize(仅租约持有者+`MIN_FIT_SIZE`+去重)、`focus_input` 四重校验(租约/回滚态/帧新鲜度≤3/重新采集剥离 ANSI 后 `terminal_input_rows`)、直连输入(9 键白名单、8ms text 合并、submit 前 `LIVE_SUBMIT_GAP_S`+`KeyInjector.ensure_submitted` 确认)。`src/web/app.py` 新增 `/api/v1/terminal/{member}/mirror` WS 端点，手动做 Host/Origin/cookie 三项鉴权(HTTP 中间件不拦截 websocket scope)。顺带发现并修:`pyproject.toml` 的 `uvicorn` 缺 `websockets`/`wsproto`，真实浏览器 WS 升级在协议层会直接失败(测试用内存传输绕过了这个问题，测试本身一直是绿的)，已换成 `uvicorn[standard]`；`tmuxctl.Tmux.capture_with_cursor` 加可选 `escape` 参数(镜像帧要颜色，原调用点默认值不变)。前端终端组件半边(§10 xterm.js 接入)留给 WEB-005 之后。只调 `control.lease`/`control.terminal`，不 import `console`(`press_key`/`submit_live_text` 等价逻辑基于 `tmuxctl` 原语重新实现，未导入 `console.control.MemberController`)。
    - 验证:`uv run ruff check .` 全过；`uv run pytest tests/test_web_terminal.py tests/test_web_snapshots.py tests/test_web_auth.py tests/test_control_plane.py tests/test_control_lease.py tests/test_tmuxctl_inject.py tests/test_console_control.py tests/test_console_members.py tests/test_release_package.py -q` 为 104 passed；`uv build --wheel` 实测 wheel 内确认新增 `web/terminal.py`。`tests/test_web_terminal.py` 用隔离 tmux socket(不碰系统默认 server)真实覆盖派工要求的四个场景:TUI/Web 同时观看共享同一广播、抢占后原持有者收到 `lease_lost`、回滚态输入被拒(`scrolled-back`)、断线后租约立即可被下一个连接拿到；另加一条 Origin 拒绝。尚未做的:真实浏览器 + 视觉自验证(§11 要求，等 WEB-005 有前端可点了再补)、`cursor_x`(§12.1 已知限制，未实现)。
  - 前置:WEB-002、WEB-005。
