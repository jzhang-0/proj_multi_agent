...
- [x] **WEB-003** — Web 后端骨架与一致 snapshot API：`amux web` 入口，FastAPI/ASGI，默认只监听 `127.0.0.1`，本机认证会话；提供 workspace/team/work/timeline/member snapshot(带 `epoch/revision`)，只调用 WEB-001 控制面。接口覆盖「顶部 Header / 工作区副标题」「左栏任务摘要」「左栏工作对话记录卡」「左栏成员卡片」「左栏任务列表」「中栏任务详情」「任务证据」「不可覆盖任务事件流」「任务关联工作对话」「工作对话记录时间线」「健康告警」「成员状态实时更新」。wheel 内带最小静态健康页；`qa.release` 源码外安装后可启动。Web 依赖加入默认依赖。
  - 处理登记:Sol，2026-08-30 18:31 +0800，分支 `web-003-fix-sol`(T-009 第三轮评审退回修复)。
  - 验证(Sonnet，2026-08-30):`uv run ruff check .` 全过；`uv run pytest tests/test_web_snapshots.py tests/test_web_auth.py tests/test_control_plane.py tests/test_console_timeline.py tests/test_console_work.py tests/test_work_ledger.py tests/test_release_package.py tests/test_control_lease.py -q` 为 72 passed(main 上重跑为 25 passed，选了 fable 指定的三个回归文件+全量 ruff)；`uv build --wheel` 实测 wheel 内确认 `web/{__init__,app,auth,cli,context,errors,snapshots,state}.py`、`web/static/health.html` 全部路径存在。
  - 证据:`src/web/app.py` 接入 `/api/v1/{session,bootstrap,vocabulary,workspace,team,work,work/tasks/{id},timeline,timeline/{seq}/body,members,health}`，全部只读、经 `require_session` 鉴权依赖(`APIRouter(dependencies=[Depends(require_session)])`，覆盖全部子路由，非逐路由手写) + 既有 Host 校验中间件，响应带 `Cache-Control: no-store`(`vocabulary` 除外)与 `application/json; charset=utf-8`；`src/web/context.py` 按 §1 规则只读解析工作区/团队/总线/tmux(直接构造 `WorkService`/`AuditLog`/`Tmux`，与 `console/app.py` 既有取资源方式一致，不触发 resize/send_keys)；`src/web/state.py` 实现 §3 epoch/revision 语义(指纹变化才 bump)与单槽时间线投影缓存；`src/web/snapshots.py` 是 §4.1-4.11 DTO 组装，投影/聚合一律经 `control.tasks`/`control.timeline`/`control.members`/`control.health`/`control.vocabulary`，不自行重算业务口径；错误按 §2.4 映射(`workspace-unregistered`/`team-unbound`/`not-found`/`invalid-request`/`work-unavailable`)。复审(fable)指出 `task_detail_view` 每次全量重建时间线(2 万行审计约 0.28s)是 `/work/tasks/{id}` 热点，已给 `control/tasks.py` 的 `task_communications`/`task_detail_view` 加可选 `timeline` 参数复用调用方已投影结果(向后兼容，既有调用不变；`seq` 仍是 `history_from_entries` 同一基准，未改动其算法，与 T-003 `6d47171` 钉住的语义一致)；评审(opus)预检发现 `timeline_snapshot_view` 上同名参数未被任何调用方使用，已删除(仅保留 `control/tasks.py` 那一处实际生效)。`tests/test_web_snapshots.py` 轻量覆盖每端点一条正常路径 + 未登记/未绑定/404/400/空账本边界。分支 `web-003-sonnet` 合入 main(`8175370`)，评审前修正合入 `53f0d74`。
