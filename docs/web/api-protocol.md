...
- **可写 WS 通道走一次性票据**(WEB-008 定案，2026-08-30)。浏览器的 `WebSocket` 构造函数**不能设置自定义请求头**，所以双提交头这条对 WS 握手用不上。可写通道(接管、直连输入等能改变成员状态的通道)改为：先由一个受双提交头保护的 HTTP 端点签发票据，WS 建连后**在首帧**把票据交上来，服务端消费后才进入可写态。硬性要求，缺一条这层就形同虚设：
  ...
  - **双模通道在"提权那一帧"交票据，不在建连首帧**(镜像通道即属此类，WEB-008 定案)。镜像 WS 一开始是纯观看(服务端 `frame`/`idle`，客户端 `scroll`)，之后才可能经 `lease acquire` 升为可写。若要求建连首帧交票，纯观看者也得先签票，与上一条的只读例外直接冲突，且多观看者场景平白多一次往返。正确做法是把票据绑在**提权动作**上：
    - **默认拒绝**：只读帧类型是**显式白名单**(服务端 `frame`/`idle`，客户端 `scroll`)，白名单之外的一律视为写，未完成提权就收到 `input`/`resize`/`focus_input` 等，一律 `close(4401)`。将来新增帧类型若忘了归类，落在"写"这一侧而不是被放行。
    - **一次票据只买一次提权**：租约被释放、被抢占或 `lease_lost` 之后，该连接退回只读；再次 `acquire` 必须交一张新票据。否则一张票等于给这条连接的整个生命周期发了通行证。
    - 票据的 `action` 绑定要能区分通道用途(镜像直连与完整接管各用各的)，一张票不得跨通道使用。
    - `/attach` 这类**全程可写**的通道仍按上一条，在建连后的首个客户端帧消费票据。
