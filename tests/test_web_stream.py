...
from web.stream import CLOSE_SLOW, DeltaRing, EventHub, StreamClient, StreamSettings
...
def test_hello_subscribe_and_timeline_work_deltas(
    ...
    with TestClient(app, base_url=f"http://127.0.0.1:{PORT}") as client:
        ...
        with _ws(client) as ws:
            ...
            assert timeline["revision"] == hello["revisions"]["timeline"] + 1
            assert timeline["ops"]
...
def test_two_clients_see_one_timeline_event_each(
    ...
    _workspace, _service, paths = _bound_workspace(tmp_path, monkeypatch)
    session = WebSession.generate()
    app = create_app(session=session, port=PORT, stream_settings=FAST)
    with TestClient(app, base_url=f"http://127.0.0.1:{PORT}") as client:
        client.get(f"/?token={session.token}")
        with _ws(client) as left, _ws(client) as right:
            ...
            deposit(Message.create("sonnet", "双端", sender="fable", task="T-001"), paths)
            app.state.stream.scan_files_now()
            for sock in (left, right):
...
def test_replay_timeline_after_disconnect(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _workspace, _service, paths = _bound_workspace(tmp_path, monkeypatch)
    session = WebSession.generate()
    app = create_app(session=session, port=PORT, stream_settings=FAST)
    with TestClient(app, base_url=f"http://127.0.0.1:{PORT}") as client:
        ...
        with _ws(client) as ws:
            ...
            deposit(Message.create("sonnet", "追平", sender="fable", task="T-001"), paths)
            app.state.stream.scan_files_now()
            first = ws.receive_json()
        ...
        with _ws(client) as ws:
            ...
            deposit(Message.create("sonnet", "再一条", sender="fable", task="T-001"), paths)
            app.state.stream.scan_files_now()
